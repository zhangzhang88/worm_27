...
const lessonDescriptions = [
  ...
  "第六集 The Playgroup（乔治第一天上幼儿园）",  
  "第七集 Mummy Pig at work（猪妈妈在工作）",
  "第八集 Piggy in the Middle!（中间的小猪！）",
  "第九集 Daddy Loses his Glasses！（爸爸的眼镜不见了）",
  "第十集 Gardening（花园种菜）",
  "第十一课 Hiccups（打嗝）",
  "第十二课 Bicycles（自行车）",
  "第十三课 Secrets（秘密）",
  "第十四课 Flying a kite（放风筝）",
  "第十五课 Picnic（野餐）",

];

const visibleLessonsCount = 15;
...
const CoursePage: React.FC = () => {
  ...
  const [exerciseCounts, setExerciseCounts] = useState<number[]>([]);
  ...
  useEffect(() => {
    setIsClient(true);
    // 在客户端加载数据
    const storedCounts = window.localStorage.getItem(`exerciseCounts_${courseId}`);
    const storedLastPracticed = window.localStorage.getItem(`lastPracticedLesson_${courseId}`);
    const storedLastClicked = window.localStorage.getItem(`lastClickedLesson_${courseId}`);

    if (storedCounts) {
      setExerciseCounts(JSON.parse(storedCounts));
    } else {
      setExerciseCounts(Array(lessonDescriptions.length).fill(0));
    }
  ...
  const handleStartLearning = useCallback((index: number) => {
    if (typeof window !== 'undefined') {
      setExerciseCounts(prevCounts => {
        ...
        window.localStorage.setItem(`exerciseCounts_${courseId}`, JSON.stringify(newCounts));
        return newCounts;
      });
      setLastPracticedLesson(index);
      setLastClickedLesson(index);
      window.localStorage.setItem(`lastPracticedLesson_${courseId}`, index.toString());
      window.localStorage.setItem(`lastClickedLesson_${courseId}`, index.toString());
          ...
          {lessonDescriptions.map((description, index) => (
            <div 
              ...
              className={`bg-[#05051d] rounded-lg p-3 shadow-md relative transition-all duration-300 ease-in-out hover:shadow-lg hover:shadow-white/50 cursor-pointer
                ${lastClickedLesson === index ? 'border-purple-500 border-2' : lastPracticedLesson === index ? 'border-white border-2' : 'border-white/50 border'} h-40 flex flex-col justify-between overflow-visible
                group`}
            >
              <LessonPreview 
                lessonIndex={index} 
