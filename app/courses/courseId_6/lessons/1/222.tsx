...
import { SentenceData } from './wordDataLesson1';
import { congratulationsMessages } from '../congratulationsData';
import { ChevronLeft, ChevronRight, Volume2, Headphones } from 'lucide-react';
...
const ButtonGroup = styled.div`
  ...
  justify-content: center;
  align-items: center;
  gap: 0.5rem; // 将间距从 1rem 减小到 0.5rem
  padding: 1rem;
...
const WordInputWrapper = styled.div<{ width: string; isLongWord: boolean }>`
  ...
  width: ${props => props.isLongWord ? `calc(${props.width} + 3em)` : `calc(${props.width} + 0.5em)`}; // 为长单词增加3em宽度
  min-width: 2em;
  transition: width 0.3s ease;
`;

const WordInput = styled.input<{ isCorrect: boolean; isWrong: boolean; isCurrent: boolean; isLocked: boolean; isSubmitted: boolean; value: string; onChange: (e: React.ChangeEvent<HTMLInputElement>) => void; onKeyDown: (e: React.KeyboardEvent<HTMLInputElement>) => void; autoFocus?: boolean }>`
  width: 100%; // 改为 100%，使其填满 WordInputWrapper
  min-width: 1.5em;
  padding: 1px 0;
  font-size: 3rem;
  border: none;
...
const WordUnderline = styled.div<{ isCorrect: boolean; isWrong: boolean; isCurrent: boolean; isPunctuation: boolean; isSubmitted: boolean; isLongWord: boolean }>`
  position: relative;
  width: ${props => props.isLongWord ? 'calc(100% + 3em)' : 'calc(100% + 0.5em)'}; // 为长单词增加3em宽度
  min-width: 2em;
  height: 2px;
...
const ChineseSentence = styled.div`
  margin-bottom: 1.5rem; // 增加这个值可以增大中文和音标之间的间隙
`;
...
export default function TypingGame({ lessonNumber, sentences: initialSentences = [], totalLessons, lessonTitle }: TypingGameProps) {
  ...
  useEffect(() => {
    if (!isDataReady || !sentences || sentences.length === 0 || currentSentenceIndex >= sentences.length) {
      return;
    }
  ...
  }, [currentSentenceIndex, sentences, showIntermediatePage, lessonNumber, isDataReady, isListeningMode]);
      ...
      const playNextSentence = async () => {
        if (listeningModeIndex < sentences.length) {
          try {
            ...
            await new Promise(resolve => setTimeout(resolve, 1000)); // 在播放下一句前等待1秒
  ...
  const calculateTextWidth = (text: string) => {
    if (textWidthRef.current) {
      ...
      return `${textWidthRef.current.offsetWidth / 16 + 1}em`; // 增加 1em 而不是 0.5em
    }
    return `${text.length + 2}em`; // 增加 2em 而不是 1.5em
  };
  ...
  const playSentenceAudio = (text: string, sequenceNumber: number): Promise<void> => {
    return new Promise((resolve, reject) => {
      ...
      const audioPath = `backend/audio_cache/courseId_6/1/`;
  ...
  const isWordCorrect = (params: WordCheckParams): boolean => {
    const normalizedInput = normalizeText(params.input);
  ...
  const checkAnswer = async () => {
    ...
    if (isCorrect) {
      ...
      setProcessedWords(prev => prev.map(word => ({
        ...word,
        isLocked: isWordCorrect({ word: word.word, input: word.input, punctuation: word.punctuation, isLocked: word.isLocked, width: word.width, originalWidth: word.originalWidth, isLongWord: word.isLongWord })
      })));
    }
  };
  ...
  const goToNextLesson = () => {
    ...
    if (nextLessonNumber <= totalLessons) {
      const nextPath = `/courses/courseId_6/lessons/${nextLessonNumber}`;
      router.push(nextPath);
    } else {
      router.push('/courses/courseId_6');
    }
  };
    ...
    return (
      <CongratulationsContainer backgroundColor={congratulationsBackgroundColor} fontColor={congratulationsFontColor}>
        ...
        <ButtonGroup>
          ...
          <Link href="/courses/courseId_6" passHref>
  ...
  return (
    <React.Fragment>
      ...
      <Container themeColor={themeColor} fontFamily={selectedFont}>
            ...
            {processedWords.map(({ word, punctuation, input, isLocked, width, isLongWord }, index) => (
              <WordInputWrapper key={index} width={width} isLongWord={isLongWord}>
                <WordInput
                  ...
                  isCorrect={isWordCorrect({ word, input, punctuation, isLocked, width, originalWidth: width, isLongWord })}
                  isWrong={input.trim() !== '' && !isWordCorrect({ word, input, punctuation, isLocked, width, originalWidth: width, isLongWord })}
        ...
        <Link href="/courses/courseId_6" passHref legacyBehavior>
