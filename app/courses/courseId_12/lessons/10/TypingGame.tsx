...
const ContentWrapper = styled.div`
  ...
  flex: 1;
  padding: 1rem;
  padding-top: 5rem;  // 增加顶部内边距
  overflow-y: auto;
  max-height: calc(100vh - 160px);  // 稍微增加最大高度
  ...
  return words.map(word => {
    ...
    if (match) {
      ...
      const baseWidth = `${baseWord.length + (
        isLongWord ? 5 : 
        isMediumWord ? 5.5 :
        isEightLetterWord ? 5.0 : 
        isSevenLetterWord ? 4.7 : 
        isSixLetterWord ? 3.3 : 
        isFiveLetterWord ? 3.0 : 
...
export default function TypingGame({ lessonNumber, sentences: initialSentences = [], totalLessons, lessonTitle }: TypingGameProps) {
  ...
  const playSentenceAudio = (text: string, sequenceNumber: number): Promise<void> => {
    return new Promise((resolve, reject) => {
      ...
      const audioPath = `backend/audio_cache/courseId_12/10/`;
  ...
  const goToNextLesson = () => {
    ...
    if (nextLessonNumber <= totalLessons) {
      const nextPath = `/courses/courseId_12/lessons/${nextLessonNumber}`;
      router.push(nextPath);
    } else {
      router.push('/courses/courseId_12');
    }
  };
    ...
    return (
      <CongratulationsContainer backgroundColor={congratulationsBackgroundColor} fontColor={congratulationsFontColor}>
        ...
        <ButtonGroup>
          ...
          <Link href="/courses/courseId_12" passHref>
  ...
  return (
    <React.Fragment>
      ...
      <Container themeColor={themeColor} fontFamily={selectedFont}>
        ...
        <Link href="/courses/courseId_12" passHref legacyBehavior>
