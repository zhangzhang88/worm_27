...
export default function TypingGame({ lessonNumber, sentences: initialSentences, totalLessons, lessonTitle }: TypingGameProps) {
  ...
  const handleInputChange = (index: number, value: string) => {
    ...
    setProcessedWords(prev => {
      ...
      newWords[index] = { 
        ...
        input: cleanedValue,
        width: newWidth,
        isLocked: false // 解锁单词，允许编辑
      };
      return newWords;
    });
    playTypingSound();

    // 如果当前单词输入完成并且正确，自动移动到下一个单词
    if (cleanedValue === processedWords[index].word) {
  ...
  const handleKeyDown = async (e: React.KeyboardEvent<HTMLInputElement>, index: number) => {
    ...
    if (e.key === 'Backspace' && inputRefs.current[index]?.value === '') {
      e.preventDefault();
      moveToPreviousWord(index);
      return;
    }

    if (e.key === 'ArrowRight' || e.key === ' ') {
      e.preventDefault();
      if (isSubmitted && !isWordCorrect(processedWords[index], true)) {
  ...
  const moveToPreviousWord = (currentIndex: number) => {
  ...
  const handleFixFirstIncorrectWord = () => {
    const firstIncorrectIndex = processedWords.findIndex(word => !isWordCorrect(word, true));
  ...
  const clearInput = () => {
    setProcessedWords(prev =>
      prev.map(word => ({ ...word, input: '', width: word.originalWidth, isLocked: false }))
    );
    setCurrentWordIndex(0);
    setIsSubmitted(false);
    setTimeout(() => {
  ...
  const isWordCorrect = (word: ProcessedWord, strictCheck: boolean = false) => {
    const normalizedInput = normalizeText(word.input);
    const normalizedWord = normalizeText(word.word);
    return strictCheck ? normalizedInput === normalizedWord : normalizedInput.startsWith(normalizedWord);
  };

  const checkAnswer = async () => {
    const isCorrect = processedWords.every(word => isWordCorrect(word, true));
    ...
    if (isCorrect) {
      ...
      setProcessedWords(prev => prev.map(word => ({
        ...word,
        isLocked: isWordCorrect(word, true)
      })));
      handleFixFirstIncorrectWord();
    }
  };
            ...
            {processedWords.map(({ word, punctuation, input, isLocked, width }, index) => (
              <WordInputWrapper key={index} width={width}>
                <WordInput
                  ...
                  isCorrect={isWordCorrect({ word, input, punctuation, isLocked, width, originalWidth: width }, true)}
                  isWrong={input.trim() !== '' && !isWordCorrect({ word, input, punctuation, isLocked, width, originalWidth: width }, true)}
