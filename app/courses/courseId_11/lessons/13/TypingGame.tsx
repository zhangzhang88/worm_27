...
import { SentenceData } from './wordDataLesson13';
...
const ContentWrapper = styled.div`
  ...
  flex: 1;
  padding: 1rem;
  padding-top: 4rem;  // 从 5rem 减少到 4rem
  overflow-y: auto;
  max-height: calc(100vh - 180px);  // 稍微增加最大高度
...
const ButtonGroup = styled.div`
  ...
  @media (max-width: 768px) {
    ...
    bottom: 20px;
    left: 0;
    background-color: ${(props: any) => (props.theme as any).backgroundColor ?? '#05051d'};
    z-index: 1000;
    padding-bottom: 20px;
    width: 100%;
  }
...
const ChineseMeaning = styled.div`
  ...
  text-align: center;
  margin-bottom: 2rem;
  margin-top: -2rem;  // 从 -3rem 改为 -2rem，减少向上的偏移
`;
...
const AnswerDisplay = styled.div<{ showAnswer: boolean }>`
  ...
  position: sticky;
  top: 0;
  background-color: ${props => (props.theme as any).backgroundColor ?? '#05051d'};
  padding: 1rem 0;
  z-index: 10;
`;
...
export default function TypingGame({ lessonNumber, sentences: initialSentences = [], totalLessons, lessonTitle }: TypingGameProps) {
  ...
  const playSentenceAudio = (text: string, sequenceNumber: number): Promise<void> => {
    return new Promise((resolve, reject) => {
      ...
      const audioPath = `backend/audio_cache/courseId_11/13/`;
  ...
  const goToNextLesson = () => {
    ...
    if (nextLessonNumber <= totalLessons) {
      const nextPath = `/courses/courseId_11/lessons/${nextLessonNumber}`;
      router.push(nextPath);
    } else {
      router.push('/courses/courseId_11');
    }
  };
    ...
    return (
      <CongratulationsContainer backgroundColor={congratulationsBackgroundColor} fontColor={congratulationsFontColor}>
        ...
        <ButtonGroup>
          ...
          <Link href="/courses/courseId_11" passHref>
    ...
    <React.Fragment>
      ...
      <Container themeColor={themeColor} fontFamily={selectedFont}>
        ...
        <AnswerDisplay showAnswer={showAnswer}>
          <AnswerText>{sentences[currentSentenceIndex].sentence}</AnswerText>
        ...
        <Link href="/courses/courseId_11" passHref legacyBehavior>
