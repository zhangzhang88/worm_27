...
import { SentenceData } from './wordDataLesson3';
...
export default function TypingGame({ lessonNumber, sentences: initialSentences = [], totalLessons, lessonTitle }: TypingGameProps) {
  ...
  }, [currentSentenceIndex, sentences, showIntermediatePage, lessonNumber, isDataReady]);
  ...
  const playSentenceAudio = async (text: string, sequenceNumber: number) => {
    ...
    const audioPath = `backend/audio_cache/courseId_5/3/`;
  ...
  const goToNextLesson = () => {
    ...
    if (nextLessonNumber <= totalLessons) {
      const nextPath = `/courses/courseId_5/lessons/${nextLessonNumber}`;
      router.push(nextPath);
    } else {
      router.push('/courses/courseId_5');
    }
  };
    ...
    return (
      <CongratulationsContainer backgroundColor={congratulationsBackgroundColor} fontColor={congratulationsFontColor}>
        ...
        <ButtonGroup>
          ...
          <Link href="/courses/courseId_5" passHref>
  ...
  return (
    <React.Fragment>
      ...
      <Container themeColor={themeColor} fontFamily={selectedFont}>
        ...
        <Link href="/courses/courseId_5" passHref legacyBehavior>
