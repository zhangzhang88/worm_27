...
import { sentences as defaultSentences, SentenceData } from './wordDataLesson1';
...
// New styled components for listening mode
...
export default function TypingGameLesson1() {
  ...
  useEffect(() => {
    const userSentences = localStorage.getItem('userSentencesLesson1');
    if (userSentences) {
      try {
    ...
    const savedIndex = localStorage.getItem('currentSentenceIndexLesson1');
    if (savedIndex) {
  ...
  const enterListeningMode = () => {
    ...
    playSentenceAudio(sentences[0].sentence, 1);
  };
  ...
  // 修改 playSentenceAudio 函数
  const playSentenceAudio = (text?: string, sequenceNumber?: number) => {
    if (sentenceAudioRef.current) {
      sentenceAudioRef.current.pause();
      sentenceAudioRef.current.currentTime = 0;
    }

    const audioPath = `/audio_1/lesson_1/${sequenceNumber || sentences[currentSentenceIndex].id}.mp3`;
    sentenceAudioRef.current = new Audio(audioPath);
    sentenceAudioRef.current.play().catch(e => {
      console.error("Error playing sentence audio:", e);
    });

    sentenceAudioRef.current.onended = () => {
      if (isListeningMode && listeningModeIndex < sentences.length - 1) {
        setListeningModeIndex(prevIndex => prevIndex + 1);
        playSentenceAudio(sentences[listeningModeIndex + 1].sentence, listeningModeIndex + 2);
      }
    };
  };
    ...
    <React.Fragment>
      ...
      {isListeningMode && (
        <ListeningModeContainer>
          ...
          <ListeningModeButton onClick={exitListeningMode} themeColor={themeColor}>
            <ButtonFrame>
              <X size={16} />
            </ButtonFrame>
