...
import { SentenceData } from './wordDataLesson20';
...
// 修改 ProcessedWord 接口，添加 isShortWord 属性
interface ProcessedWord {
  word: string;
  punctuation: string;
  input: string;
...
const processSentence = (sentence: string): ProcessedWord[] => {
  ...
  return words.map(word => {
    ...
    if (match) {
      ...
      const isLongWord = baseWord.length > 8;
      const isMediumWord = baseWord.length === 8;
      const isShortWord = baseWord.length <= 2;
      const baseWidth = `${baseWord.length + (isLongWord ? 5 : isMediumWord ? 4 : isShortWord ? 1 : 3)}em`;
      return { 
        word: baseWord, 
        punctuation: match[2], 
        input: '', 
    ...
    const isMediumWord = word.length === 8;
    const isShortWord = word.length <= 2;
    const baseWidth = `${word.length + (isLongWord ? 3.5 : isMediumWord ? 2.5 : isShortWord ? 0.5 : 1.5)}em`;
    return { 
      word, 
      punctuation: '', 
      input: '', 
