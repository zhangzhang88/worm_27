...
export default function TypingGameLesson1() {
        ...
        if (listeningModeIndex < sentences.length) {
          try {
            for (let i = 0; i < playCount; i++) {
              await playSentenceAudio(sentences[listeningModeIndex].sentence, listeningModeIndex + 1);
              if (i < playCount - 1) {
  ...
  }, [isListeningMode, isPlayingListeningMode, listeningModeIndex, sentences, playCount, intervalTime]);
  ...
  const playSentenceAudio = (text?: string, sequenceNumber?: number) => {
    if (!text || !sequenceNumber) {
      if (sentences.length > 0 && currentSentenceIndex < sentences.length) {
        const audioSrc = `/audio_1/lesson_1/${sentences[currentSentenceIndex].id}.mp3`;
      ...
      return new Promise<void>((resolve, reject) => {
        const audioPath = `/audio_1/lesson_1/${sequenceNumber}.mp3`;
  ...
  // New functions for listening mode
