...
import { ChevronLeft, ChevronRight, Volume2, Headphones, Play, Pause, X } from 'lucide-react';
...
export default function TypingGame({ lessonNumber, sentences: initialSentences = [], totalLessons, lessonTitle }: TypingGameProps) {
  ...
  // 新增的听力模式效果
  useEffect(() => {
    if (isListeningMode && isPlayingListeningMode && !isPaused) {
  ...
  }, [isListeningMode, isPlayingListeningMode, listeningModeIndex, sentences, isPaused]);
      ...
      {isListeningMode && (
        <ListeningModeContainer>
          <ListeningModeControlsContainer>
            <ListeningModeControlButton onClick={restartListeningMode} themeColor={themeColor}>
              <Play size={16} />
            </ListeningModeControlButton>
            <ListeningModeControlButton onClick={togglePause} themeColor={themeColor}>
