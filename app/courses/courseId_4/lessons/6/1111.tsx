...
const ButtonGroup = styled.div`
  ...
  @media (max-width: 768px) {
    ...
    justify-content: space-between; // 左右按钮放在两边
  }
`;
...
export default function TypingGame({ lessonNumber, sentences: initialSentences, totalLessons, lessonTitle }: TypingGameProps) {
  ...
  // 使用 useEffect 确保在客户端访问 window
  useEffect(() => {
    if (typeof window !== 'undefined') {
      // 这里可以安全地使用 window 对象
      const progressText = `${currentSentenceIndex + 1}/${sentences.length}`;
      console.log(progressText); // 示例：在控制台输出进度
    }
    ...
    <React.Fragment>
      ...
      <Container themeColor={themeColor} fontFamily={selectedFont}>
        ...
        <ButtonGroup>
          ...
          <Button onClick={() => setShowIntermediatePage(true)} themeColor={themeColor}>
            <ButtonFrame>
